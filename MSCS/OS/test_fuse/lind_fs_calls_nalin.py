...
#---------------------------------------------
#linddata.x is where my data goes to show logical filesystem.

FILEDATAPREFIX = 'linddata.'

#might not use df_checker. I have replaced dir_or_file_checker to 'f' or 'd' to decide what it is as directed in class. but this was my initial implementation. i am not using it but i am comfortable to have it around.

checker=0
df_checker=0
dir_or_file_checker=df_checker
mycontext={}
#action begins! 
super_block={}
root_inode={}
...
#freeend=28
mycontext={'freestart':27,'freeend':27,'link_count':2}

super_block={'ctime':2132,'mounted':2, 'devid':20, ' free_start':mycontext['freestart'], 'free_end':mycontext['freestart'], 'root' :26, 'max_block':10000}

string=serializedata(super_block)
"""try:
	removefile(super_block)
except FileNotFoundError:
	pass
"""
metadatafo=openfile(FILEDATAPREFIX+str(0),True)
metadatafo.writeat(string,0)
metadatafo.close()
	
#now i declare root

root_inode={'size':4096, 'uid':20, 'gid':20, 'mode': S_IFDIR|S_IRWXA,'atime':12123,'ctime':121331, 'linkcount':mycontext['link_count'],'filename_to_inode_dict':{'.':26,'..':26 }} 
mycontext['link_count']=len(root_inode['filename_to_inode_dict'])
string=serializedata(root_inode)
"""try:
	removefile(root_inode)
except FileNotFoundError:
	pass
"""
metadataro=openfile(FILEDATAPREFIX+str(26),True)
metadataro.writeat(string,0)
metadataro.close()
#-----------------------------------------------------------------
...
def mkdir_syscall(path, mode):
  ...
  try:
    ...
    #assuming 'dirname is the actual name then updating root
    #updating stuff in root
    mycontext['link_count']=mycontext['link_count']+1
	
    mycontext['freestart']=mycontext['freestart']+1
    root_inode['filename_to_inode_dict'][dirname]=mycontext[0]
    root_inode['linkcount']=mycontext['link_count']
  #  mycontext['linkcount']=len(root_inode['filename_to_inode_dict'])
    dir_inode={'size':4096, 'uid':20, 'gid':20, 'mode': S_IFDIR|S_IRWXA,'atime':12123,'ctime':45456, 'linkcount':2,'filename_to_inode_dict':{'.':mycontext['freestart'],"..":mycontext['freestart']} } 
    ...
    string=serializedata(dir_inode)
    try:
 	removefile(dir_inode)
    except FileNotFoundError:
 	pass
    metadatado=openfile(FILEDATAPREFIX+str(mycontext['freestart']),True)
    metadatado.writeat(string,0)
    metadatado.close()
...
# okay, great!!!   We're ready to go!   Let's make the new directory...
    ...
    mycontext['link_count']=mycontext['link_count']-1
    mycontext['freestart']=mycontext['freestart']-1
    del root_inode['filename_to_inode_dict'][dirname]
    root_inode['linkcount']=mycontext['link_count']
...
def link_syscall(oldpath, newpath):
  ...
  try:
    ...
    #-----------------------------------------------
    index={}
    Number_of_blocks_needed=raw_input("size of file?")
    Number_of_blocks_needed=int(filesize/4096)
    if Number_of_blocks_needed>1:
	file_inode['indirect']=1
	file_inode['fnext_index']=mycontext['freestart']+1
	string=serializedta(file_inode)
	try:
		removefile(file_inode)
	except FileNotFoundError:
		pass
	metadatafo=openfile(FILEDATAPREFIX+str(mycontext['freestart']),True)
	metadatafo.writeat(string,0)
	metadatafo.close()
	mycontext['freestart']=file_inode['fnext_index']+1
			
	for i in range(mycontext['freestart'],mycontext['freestart']+Number_of_blocks_needed):
		for k in range(0,(mycontext['freestart']+Number_of_blocks_needed)-(mycontext['freestart']+1)):
			index['data']=i
			string=serializedta(index)
			try:
				removefile(index)
			except FileNotFoundError:
				pass
			metadatafo=openfile(FILEDATAPREFIX+str(i),True)
			metadatafo.writeat(string,0)
			metadatafo.close()
